...
| 阶段 | 时间（北京时间）              | 候选人数 | 上游名单文件 |
| ---- | ----------------------------- | -------- | ------------ |
| 5    | 2026/05/05 12:00 - 05/08 23:59 | 71（女 48 / 男 23）— 第一轮 | characterOld.js |
| 6    | 2026/05/09 22:00 - 05/12 23:59 | 40（女 25 / 男 15）— 第二轮（第一轮 TOP 入围） | characterb.js |
| 7    | 2026/05/13 12:00 - 05/15 23:59 | 59（女 44 / 男 15）— 复活赛 | characterc.js |
| 8    | 2026/05/16 12:00 - 05/19 23:59 | 16（女 7 / 男 9）— 决赛 | characterd.js |

阶段之间会有"投票未开启"窗口，那时上游 `lstV` 字段为空，所有人显示 0 票，是正常的（程序会提示）。
程序每小时会自动从 `index.html` 嗅探出当前阶段对应的 `character*.js` 并拉取最新名单，跨阶段无需重启。
...
程序运行时每小时自动嗅探 `index.html` 找到当前阶段对应的 `character*.js` 并拉取最新名单。手动刷新：

```powershell
python extract_characters.py            # 自动识别当前阶段（推荐）
python extract_characters.py --full     # 强制用全历史角色（71 人，调试用）
```
